// One-off CommonJS port of prisma/seed-finance.ts that runs under plain `node`
// (the .ts version needs ts-node, which doesn't load .ts files under the
// project's "type":"module" ESM config). Use this if ts-node fails:
//
//   node prisma/seed-finance.cjs
//
// Safe to re-run (upserts by code).

const { PrismaClient, AccountType, AccountCategory, TaxType } = require('../lib/generated-prisma');
...
const taxRates = [
    { name: 'VAT Standard Rate', code: 'VAT18', rate: 18, isPercentage: true, taxType: TaxType.VAT, description: 'Uganda Revenue Authority Standard VAT Rate 18%', effectiveFrom: new Date('2020-01-01') },
    { name: 'VAT Exempt (Medical)', code: 'VAT0', rate: 0, isPercentage: true, taxType: TaxType.VAT, description: 'VAT exempt for medical services as per Uganda VAT Act', effectiveFrom: new Date('2020-01-01') },
    { name: 'Withholding Tax 6%', code: 'WHT6', rate: 6, isPercentage: true, taxType: TaxType.WITHHOLDING, description: 'Withholding tax on professional services (URA)', effectiveFrom: new Date('2020-01-01') },
    { name: 'NIL Tax Rate', code: 'NIL', rate: 0, isPercentage: true, taxType: TaxType.NIL, description: 'No tax applicable', effectiveFrom: new Date('2020-01-01') },
];

async function seedFinance() {
    console.log('Seeding tax rates...');
    for (const tr of taxRates) {
        await prisma.taxRate.upsert({ where: { code: tr.code }, update: tr, create: tr });
    }
    ...
    console.log(`  -> ${chartOfAccounts.length} accounts`);

    const currentYear = new Date().getFullYear();
    console.log(`Seeding fiscal year FY ${currentYear}/${currentYear + 1}...`);
    ...
    console.log('  -> fiscal year + 12 monthly periods');

    console.log('\nFinancial seed complete.');
}

seedFinance()
    .catch(e => { console.error('Finance seed failed:', e); process.exit(1); })
